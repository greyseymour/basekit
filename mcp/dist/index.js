...
import { createPublicClient, http, formatEther, isAddress, parseAbi, namehash, keccak256, toHex, } from "viem";
...
const tools = [
    {
        ...
        description: "Build an activity profile for a Base address. Returns tx count, age, balance, failed-tx ratio, and top counterparties. Safe to run unbounded — read-only, no signing.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Decode a Base transaction. Returns receipt fields, decoded function selector, gas data, and tracer URLs. Read-only.",
        inputSchema: {
            type: "object",
    ...
    },
    {
        ...
        description: "Scan ERC-20 Approval events outgoing from an address on Base. Flags unlimited approvals to non-allowlisted contracts. Returns revoke URLs. Read-only.",
        inputSchema: {
            type: "object",
            properties: {
                address: { type: "string" },
    ...
    },
    {
        ...
        description: "Resolve a Basename (e.g. 'jesse.base.eth') to an address via the L2 resolver, or reverse-resolve an address. Read-only.",
        inputSchema: {
            type: "object",
    ...
    },
    {
        ...
        description: "Run a full safety report on a Base contract. Checks source verification, ownership, mint/pause/blacklist selectors, proxy status, age, GoPlus score. Returns verdict (safe/caution/danger/unknown) + structured warnings. Use before any agent calls a contract on a user's behalf.",
        inputSchema: {
            type: "object",
    ...
    {
        name: "simulate_swap",
        description: "Quote a token swap on Base via Uniswap V3 Quoter. Returns expected output, price impact, and route. Read-only — does NOT execute.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Assemble a portfolio snapshot for a Base address — ETH balance, ERC-20 holdings, recent activity. Read-only.",
        inputSchema: {
            type: "object",
            properties: {
                address: { type: "string" },
    ...
    {
        name: "gas_now",
        description: "Current Base base-fee + sequencer health. Use to decide whether to send a tx now or wait.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "decode_calldata",
        description: "Decode raw calldata into function selector + arguments. Cross-checks against 4byte directory.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Check if a basename is available to register. Returns availability + estimated cost (USD + ETH).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Preflight an ERC-20 deploy on Base. Returns estimated gas, cost in ETH at current base fee.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Preflight a Safe multisig deploy. Returns estimated gas and cost based on owner count.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Given a user's signed agent policy (JSON), produce a human-readable summary suitable for confirmation UI. Read-only.",
        inputSchema: {
            type: "object",
            properties: {
                policy: {
...
// ─── Handlers ──────────────────────────────────────────────────────────────────
async function getWalletProfile(args) {
...
async function getTransactionTrace(args) {
    const { tx_hash } = args;
...
async function checkTokenApprovals(args) {
    ...
    const rawCall = (fromBlock, toBlock) => client.request({
        method: "eth_getLogs",
        params: [
            {
    ...
    try {
        ...
        logs = await rawCall(tip - 50000n, tip);
    }
...
async function resolveBasenameRaw(input) {
    if (isAddress(input))
        return input;
...
async function resolveBasename(args) {
    const { input } = args;
    if (isAddress(input)) {
...
async function verifyContract(args) {
    ...
    let goplus = null;
    try {
        const gp = await fetch(`https://api.gopluslabs.io/api/v1/token_security/8453?contract_addresses=${address}`);
    ...
    const warnings = [];
    if (!hasCode)
        warnings.push("No bytecode at address — EOA or self-destructed.");
    if (!sourceVerified)
        warnings.push("Source code not verified on Basescan.");
    if (found.blacklist)
        warnings.push("Bytecode contains blacklist selector — critical risk.");
    ...
    if (found.upgrade_to)
        warnings.push("Upgradeable proxy — implementation can change.");
    if (ownerType === "EOA")
        warnings.push("Owner is an EOA, not a multisig.");
    if (ageDays !== null && ageDays < 7)
        warnings.push(`Contract is only ${ageDays} days old.`);
    let verdict = "unknown";
...
async function simulateSwap(args) {
    ...
    try {
        ...
        const [amountOut, , ticksCrossed, gasEstimate] = result.result;
        return {
            venue: "uniswap-v3",
            token_in,
            token_out,
        ...
        return {
            venue: "uniswap-v3",
            error: e.message,
            hint: "Try a different fee_tier (100, 500, 3000, 10000) — pool may not exist at this tier.",
        };
    }
}
async function portfolioSnapshot(args) {
    const { address } = args;
    ...
    if (ALCHEMY_API_KEY) {
        try {
            const r = await fetch(`https://base-mainnet.g.alchemy.com/v2/${ALCHEMY_API_KEY}`, {
                method: "POST",
            ...
            tokens =
                j.result?.tokenBalances?.filter((t) => t.tokenBalance !== "0x0") ?? [];
        }
        catch {
            // ignore
        }
    ...
    return {
        ...
        erc20_holdings: tokens.slice(0, 25).map((t) => ({
...
async function decodeCalldata(args) {
    const { calldata } = args;
    ...
    let signatures = [];
    try {
        const r = await fetch(`https://www.4byte.directory/api/v1/signatures/?hex_signature=${selector}`);
        const j = await r.json();
        signatures = (j.results ?? []).map((x) => x.text_signature);
    }
    catch {
        // ignore
    }
...
async function checkBasenameAvailable(args) {
    ...
    const annualUsd = len === 3 ? 100 : len === 4 ? 25 : len <= 9 ? 5 : 0.5;
...
async function estimateTokenDeploy(args) {
    const gasUnits = args.mintable ? 850000n : 700000n;
    const block = await client.getBlock();
    const baseFee = block.baseFeePerGas ?? 0n;
    ...
    return {
        ...
        note: "Heuristic estimate. Run `forge script --simulate` for an exact number tied to your bytecode.",
    };
}
async function estimateSafeDeploy(args) {
...
async function agentPolicyDescribe(args) {
    const p = args.policy ?? {};
    const lines = [];
    if (p.user_address)
        lines.push(`User: ${p.user_address}`);
    if (p.agent_address)
        lines.push(`Agent session key: ${p.agent_address}`);
    if (p.valid_until) {
    ...
    if (perms.max_per_tx_usd)
        lines.push(`Max per transaction: $${perms.max_per_tx_usd}`);
    if (perms.max_per_day_usd)
        lines.push(`Max per day: $${perms.max_per_day_usd}`);
    if (perms.allowed_skills?.length)
        lines.push(`Allowed skills: ${perms.allowed_skills.join(", ")}`);
    if (perms.allowed_contracts?.length)
        lines.push(`Allowed contracts: ${perms.allowed_contracts.length} entries`);
    if (perms.require_human_confirm_above_usd)
        lines.push(`Human confirm required above: $${perms.require_human_confirm_above_usd}`);
...
// ─── Server wiring ─────────────────────────────────────────────────────────────
const server = new Server({ name: "basekit-mcp", version: "0.2.0" }, { capabilities: { tools: {} } });
    ...
    try {
        ...
        switch (name) {
            case "get_wallet_profile":
                result = await getWalletProfile(args);
                break;
            case "get_transaction_trace":
                result = await getTransactionTrace(args);
                break;
            case "check_token_approvals":
                result = await checkTokenApprovals(args);
                break;
            case "resolve_basename":
                result = await resolveBasename(args);
                break;
            case "verify_contract":
                result = await verifyContract(args);
                break;
            case "simulate_swap":
                result = await simulateSwap(args);
                break;
            case "portfolio_snapshot":
                result = await portfolioSnapshot(args);
                break;
            case "gas_now":
                result = await gasNow();
                break;
            case "decode_calldata":
                result = await decodeCalldata(args);
                break;
            case "check_basename_available":
                result = await checkBasenameAvailable(args);
                break;
            case "estimate_token_deploy":
                result = await estimateTokenDeploy(args);
                break;
            case "estimate_safe_deploy":
                result = await estimateSafeDeploy(args);
                break;
            case "agent_policy_describe":
                result = await agentPolicyDescribe(args);
                break;
            default:
